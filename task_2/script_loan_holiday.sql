...
as $$
	begin
		...
		select  deal_rk
	   			...
	   			,effective_from_date
	   			,effective_to_date
		from RD.deal_info), loan_holiday as (
		select  deal_rk
	   		...
	   		,effective_from_date
	   		,effective_to_date
		from RD.loan_holiday), product as (
		select distinct product_rk
	  		,product_name
	  		,effective_from_date
	  		,effective_to_date
		from RD.product), holiday_info as (
		select   d.deal_rk
		...
		left join product p on p.product_rk = d.product_rk  and p.product_name=d.deal_name
